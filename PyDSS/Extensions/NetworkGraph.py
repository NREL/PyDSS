...
class CreateGraph:
    ...
    def CreateGraphVisualization(self, Settings = None):
        ...
        if NodeSize:
            C = plot.circle('X', 'Y', source=self.DataSource, size=NodeSize, level='overlay', color='colors',
                            legend='Nodes')
        else:
            C = plot.circle('X', 'Y', source=self.DataSource, level='overlay', color='colors',   legend='Nodes')

        L = plot.multi_line(Xs, Ys, color=LineColors, legend='Edges')
        plot.legend.location = "top_left"
        plot.legend.click_policy = "hide"
        output_file(os.path.join(Settings['OutputPath'] , Settings['OutputFile']))
        doc = curdoc()
        doc.add_root(plot)
        doc.title = "PyDSS"
